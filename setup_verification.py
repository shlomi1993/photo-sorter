...
def install_dependencies():
    ...
    if not success:
        ...
        # Try pyexiv2 separately as it might fail on some systems
        print("Attempting to install pyexiv2 (may fail on some systems)...")
        pyexiv2_success = run_command([str(python_exe), "-m", "pip", "install", "pyexiv2>=2.11.0"],
                                     "Installing pyexiv2")

        if not pyexiv2_success:
            print("⚠️  pyexiv2 installation failed - this is normal on some systems")
            print("   The tool will work with reduced EXIF metadata capabilities")
...
def test_installation():
    ...
    test_import = run_command([
        str(python_exe), "-c",
        "from photo_sorting import extract_date_from_directory, MetadataReader, MetadataWriter, setup_logger; print('All imports successful!')"
    ], "Testing imports")
